class Node:
    ...
    def get_exp_health_utility(self):
        """ abstract method to be overridden in derived classes
        :returns expected cost of this node """
        raise NotImplementedError("This is an abstract method and needs to be implemented in derived classes.")
...
class ChanceNode(Node):
    ...
    def get_exp_health_utility(self):
        ...
        i = 0
        for node in self.futureNodes:
            exp_health_utility += self.probs[i]*node.get_exp_health_utility()
            i += 1
...
class DecisionNode(Node):
    ...
    def get_exp_health_utility(self):
        ...
        outcomes = dict() # dictionary to store the expected cost of future nodes along with their names as keys
        for node in self.futureNode:
            outcomes[node.name] = node.get_exp_health_utility()
...
print('Utility;', D1.get_exp_health_utility())